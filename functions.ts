...
function abcd(name:string, callback: ()=> void){
    callback()
    console.log(name);
    //  name
}
...
function addperson(name:string, age:number, gender?:string):void{
    details.push({
        name,age,gender
    })
    prindetails(details)
...
function prindetails(details:Person[]):void{
    details.forEach((ele)=>{
        console.log(ele);
        console.log("\n");
...
// using  as rest para
function findmulti(...args:number[]):number{
    let sm=0;
    args.forEach((ele)=>{
        sm+=ele;
